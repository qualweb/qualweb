    ...
    if (clonedEvaluation) {
      switch (module) {
        ...
        case "wcag-techniques":
          this.modules["wcag-techniques"] = <WCAGTechniquesReport>(
            clonedEvaluation
          );
          break;
        case "best-practices":
          this.modules["best-practices"] = <BestPracticesReport>(
            clonedEvaluation
          );
          break;
      }
      ...
      if (module !== "wappalyzer") {
        ...
        this.metadata.addWarningResults(
          this.modules[module]?.metadata.warning || 0
        );
        this.metadata.addFailedResults(
          this.modules[module]?.metadata.failed || 0
        );
        this.metadata.addInapplicableResults(
          this.modules[module]?.metadata.inapplicable || 0
        );
      }
    }
  }
