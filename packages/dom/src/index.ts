...
class Dom {
  ...
  public async getDOM(browser: Browser, options: QualwebOptions, url: string, html: string): Promise<PageData> {
    try {
      ...
      if (url) {
        ...
        let sourceHTMLPuppeteer = '';
        
        if (response) {
          sourceHTMLPuppeteer = await response.text();
        }
      ...
      const allElements = CSSselect('[style]', sourceHtml.html.parsed);
      let k = 0;
  ...
  private async parseStylesheets(plainStylesheets: any): Promise<CSSStylesheet[]> {
    const stylesheets = new Array<CSSStylesheet>();
    ...
    for (const file in plainStylesheets || {}) {
      ...
      if (stylesheet.content) {
        ...
        stylesheet.content.parsed = css.parse(plainStylesheets[file], {
          silent: true
  ...
  private async processShadowDom(): Promise<void> {
    let selectors = await this.page.evaluate((elem) => {
      ...
      const listElements = document.querySelectorAll('*') || new Array();
