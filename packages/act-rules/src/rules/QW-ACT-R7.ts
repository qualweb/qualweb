...
class QW_ACT_R7 extends Rule {
  ...
  public async unmappedExecute(styleSheets: CSSStylesheet[]): Promise<void> {
    ...
    if(styleSheets.length === 0){
      ...
      if(foundATransform){
        let keys = Object.keys(this.mediaMap)
        for(let i = 0; i < keys.length; i++){
          let angle = this.mediaMap[keys[i]].replace("rotate(", "").replace(")", "");
  ...
  private parseDegrees(angle: string): number{
    ...
    if(angle.includes("deg")){
      ...
      let radians = parseFloat(angle.replace("rad", ""));
      return radians * 180 / Math.PI;
    }else if(angle.includes("turn")){
      let turnDegrees = 360;
      let turns = parseFloat(angle.replace("turn", ""));
