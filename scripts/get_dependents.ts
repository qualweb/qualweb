...
async function main(): Promise<void> {
  ...
  if (command.opts().includeSelf !== true) dependents = dependents.filter((dep) => dep !== packageName);

  if (command.opts().asWorkspaces === true) console.debug(dependents.map((dep) => `-w ${dep}`).join(' '));
  else console.debug(dependents);
}
