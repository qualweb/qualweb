 ...
 * has been changed, will output @qualweb/core, @qualweb/act-rules.
 * Will mention a package at most once.
 */
...
const command = new Command('get-modified-workspaces')
  .option(
    '--as-workspaces',
    'Outputs the results as a list of workspace parameters fit for consumption by npm and npx.'
  )
  .option('--input-separator <separator>', 'Separator between filenames in the input file.', ' ')
  .option(
    '--output-separator <outputSeparator>',
    'Separator between workspace entries. This is ignored when using --as-workspaces',
    ' '
  )
  .argument(
    '<path-to-modified_files.txt>',
    'Input file. Each entry should be separated by the value of --separator (defaults to a whitespace)'
  );
...
async function main(): Promise<void> {
  ...
  if (command.opts().asWorkspaces === true) console.debug(finalResult.map((dep) => `-w ${dep}`).join(' '));
  else console.debug(finalResult.join(opts.outputSeparator));
}
